...
class DecisionTreeParameters(BaseModel):
    ...
    max_depth: Optional[int] = Field(5, ge=1, description="Maximum depth of the tree")
    ...
    def to_sklearn_params(self) -> Dict[str, Any]:
        ...
        params = self.model_dump(exclude_none=True, exclude={"dataset"})
        return params
...
# Union type for tree nodes with discriminator
TreeNode = Annotated[Union[SplitNode, LeafNode], Field(discriminator="type")]
...
class NodeStatParameters(BaseModel):
    ...
    parent_samples_mask: Optional[List[int]] = Field(
        None, description="Indices of samples that reached this node (None for root)"
    )
    criterion: str = Field(
        default="gini",
...
class ManualFeatureStatsParameters(BaseModel):
    ...
    max_thresholds: int = Field(
        default=100, ge=10, le=500, description="Maximum number of thresholds to return"
    )
