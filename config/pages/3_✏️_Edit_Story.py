            ...
            for i in range(num_edges):
                with st.expander(f"Edge {i+1}", expanded=(i==0)):
                    ...
                    with col_a:
                        ...
                        start_local_index = st.number_input("Local Index", min_value=0, value=default_start_idx, key=f"start_idx_{i}")
                        default_start_story = existing_edge.get("start", {}).get("story_name", "") if existing_edge else ""
                        start_story_name = st.text_input("Story Name (optional)", value=default_start_story, key=f"start_story_{i}", placeholder="Leave empty for current story")

                    with col_b:
                        st.markdown("**End Node**")
                        default_end_idx = existing_edge.get("end", {}).get("local_index", 0) if existing_edge else 0
                        end_local_index = st.number_input("Local Index", min_value=0, value=default_end_idx, key=f"end_idx_{i}")
                        default_end_story = existing_edge.get("end", {}).get("story_name", "") if existing_edge else ""
                        end_story_name = st.text_input("Story Name (optional)", value=default_end_story, key=f"end_story_{i}", placeholder="Leave empty for current story")
